...
export async function getChefReport(chefId: number, period: Period) {
  ...
  const updatedOrders = orders.filter(o => o.updatedAt.getTime() !== o.createdAt.getTime());

  const avgCompletionTimeMinutes =
    completedOrders.reduce((acc, o) => acc + (o.updatedAt.getTime() - o.createdAt.getTime()), 0) /
