...
const classes = {
  ...
  title: "text-xs uppercase tracking-widest font-semibold text-black/60 sm:text-sm md:text-base truncate",
  value: "text-2xl font-extrabold mt-1 sm:text-3xl md:text-4xl truncate my-2",
  subtitle: "text-sm mt-0.5 text-black/60 sm:text-base md:text-lg line-clamp-1",

  iconWrapper:
    "flex items-center justify-center w-10 h-10 sm:w-12 sm:h-12 rounded-lg border border-black/10",

  trend: "flex items-center gap-1 mt-2 text-xs sm:text-sm md:text-base",
  trendValue: "font-semibold",

  progressWrapper: "mt-3",
  progressHeader: "flex justify-between text-xs font-medium mb-1 sm:text-sm md:text-base",
  progressTrack: "h-1.5 w-full rounded-full bg-black/10 overflow-hidden",
  progressBar: "h-full bg-black",

  loadingWrapper: "absolute top-0 left-0 right-0 h-1 bg-black/10 overflow-hidden",
  loadingBar: "h-full w-1/3 animate-pulse bg-black",
};
...
const DynamicCard: React.FC<StatsCardProps> = ({
  ...
  const isPositive = trend ? trend.isPositive ?? trend.value > 0 : false;
  const TrendIcon = trend?.value === 0 ? Minus : isPositive ? TrendingUp : TrendingDown;

  return (
    <CardContainer className={classes.container}>
      <div onClick={onClick} className={onClick ? classes.clickableWrapper : undefined}>
        <CardBody className={classes.cardBody}>
          ...
          {trend && (
            <CardItem translateZ={30} className={classes.trend}>
              <TrendIcon size={12} />
              <span className={classes.trendValue}>{Math.abs(trend.value)}%</span>
