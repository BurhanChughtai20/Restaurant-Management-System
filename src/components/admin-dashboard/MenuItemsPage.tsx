...
import {
  DynamicTable, DynamicCard, Column, TableAction
...
const MenuItemsPage: React.FC = () => {
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      avgPrice: total > 0 ? menuItems.reduce((s, i) => s + i.price, 0) / total : 0
    };
  }, [menuItems]);
  ...
  const columns: Column<MenuItem>[] = useMemo(() => [
    ...
    { id: 'price', label: 'Price', minWidth: 100, format: (v) => `$${Number(v).toFixed(2)}` },
    { 
      id: 'isActive', 
      label: 'Status', 
  ...
  const actions: TableAction<MenuItem>[] = useMemo(() => [
    { 
      icon: <Edit size={18} />, 
      label: 'Edit', 
      onClick: (row) => row && handleOpenDialog(row) 
    },
    { 
      icon: <Trash2 size={18} />, 
      label: 'Delete', 
  ...
  return (
    <Box sx={{ p: { xs: 2, md: 4 }, bgcolor: '#f4f6f8', minHeight: '100vh' }}>
      ...
      <Box sx={{ 
        ...
        <Typography variant="h4" sx={{ fontWeight: 800, color: '#111827' }}>Menu Inventory</Typography>
        <ButtonCom
          text="Add New Item"
          type="default"
          gradient
      ...
      {/* Stats Area - Using Box Flex instead of Grid for MUI v5 compatibility */}
      <Box sx={{ 
        display: 'flex', 
        flexWrap: 'wrap', 
        gap: 3, 
